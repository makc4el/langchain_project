...
class SalesforceCredentials(BaseModel):
    """Enhanced Salesforce credentials for MCP server authentication"""
    instanceUrl: str
...
class MCPSalesforceClient:
    ...
    def set_credentials(self, 
                       ...
                       scope: Optional[str] = None):
        """Set comprehensive Salesforce credentials for API calls"""
        ...
        logger.info(f"Credentials set for instance: {normalized_instance_url}")
        logger.info(f"Using: {'access token' if access_token else 'auth code'}, token_type: {token_type}")
        if refresh_token:
            logger.info(f"Refresh token available, scope: {scope or 'not specified'}")
    
    def set_credentials_from_dict(self, credentials_dict: Dict[str, Any]):
        """Set credentials from a dictionary (convenient for JSON input)"""
        self.set_credentials(
            instance_url=credentials_dict.get('instanceUrl', ''),
            access_token=credentials_dict.get('accessToken'),
            auth_code=credentials_dict.get('authCode'),
            token_type=credentials_dict.get('tokenType', 'Bearer'),
            refresh_token=credentials_dict.get('refreshToken'),
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # For listing tools, we don't need Salesforce credentials
            headers = {
                "Content-Type": "application/json"
            }
